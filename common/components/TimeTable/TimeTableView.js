    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          />
          <Text style={{position: 'absolute', top: 5, left: 0, color: 'white', fontSize: 11}}>Time</Text>
        </View>
            ...
            <View style={styles.timeColumn}>
              {this.times.map(time => (
                <View key={time} style={styles.timeLabel}>
                  <Text style={[styles.timeText, {fontSize: 9, alignSelf:'flex-start'}]}>{time === 12 ? 12 : time % 12}:00</Text>
                  <Text style={{alignSelf: 'center', fontSize: 10, fontStyle: 'italic'}}>{time <= 12 ? 'am' : 'pm'}</Text>
                </View>
              ))}
            </View>
            <View
